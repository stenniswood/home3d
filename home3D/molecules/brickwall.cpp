...
void glBrickWall::set_length_height   ( float mLength, float mHeight )       // this or set_params()
{
    ...
    m_number_of_rows = mHeight / get_brick_height();
}

void glBrickWall::set_params          ( int mNumBricks, int mNumRows )
{
    m_wall_length    = mNumBricks * get_brick_length();
...
float	glBrickWall::get_brick_depth( )
{
    if (m_brick_type==CEMENT_BLOCK_ID)
        return CEMENT_BLOCK_THICKNESS;
    else if (m_brick_type==FACE_BRICK_ID)
        return FACE_BRICK_THICKNESS;
    return 0;
}
...
txtContainer* glBrickWall::create_one_brick( bool mHalf )
{
    ...
    mb->width  = get_brick_length();
    mb->height = get_brick_height();
    mb->depth  = get_brick_depth();
    if (mHalf)
